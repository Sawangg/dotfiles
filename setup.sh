...
if [ "$IS_ARCH_BASED" = "true" ]; then
  ...
  read -r answer
  if [ "$answer" = "y" ] || [ "$answer" = "Y" ]; then
    $SUDO pacman -S ghostty fuzzel grim slurp wl-clipboard libnotify swaync brightnessctl playerctl hyprland hyprpicker hypridle hyprlock hyprsunset keepassxc ttf-dejavu-nerd ttf-jetbrains-mono-nerd
    IS_DESKTOP="true"
  fi
else
